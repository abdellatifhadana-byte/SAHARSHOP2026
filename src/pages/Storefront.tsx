    ...
    <div onClick={()=>onView(p)} onMouseEnter={()=>{setHover(true);if(imgs.length>1)setImgIdx(1);}} onMouseLeave={()=>{setHover(false);setImgIdx(0);}}
      ...
      <div style={{aspectRatio:'3/4',position:'relative',background:DS.bgGlass,overflow:'hidden'}}>
        ...
        <div style={{position:'absolute',top:12,left:12,display:'flex',gap:6,flexWrap:'wrap'}}>
          {isNew&&<span style={{background:`linear-gradient(135deg, ${DS.purple}, ${DS.purpleLight})`,color:'#fff',fontSize:10,fontWeight:700,padding:'4px 10px',borderRadius:DS.radiusFull,boxShadow:`0 2px 8px ${DS.purpleGlow}`}}>جديد</span>}
          ...
          {p.stock<=3&&p.stock>0&&<span style={{background:DS.orangeSoft,backdropFilter:'blur(8px)',color:DS.orangeLight,fontSize:10,fontWeight:700,padding:'4px 10px',borderRadius:DS.radiusFull,border:`1px solid ${DS.borderOrange}`}}>آخر {p.stock}</span>}
          {p.stock===0&&<span style={{background:'rgba(255,255,255,0.06)',backdropFilter:'blur(8px)',color:DS.textTertiary,fontSize:10,fontWeight:700,padding:'4px 10px',borderRadius:DS.radiusFull,border:DS.glassBorder}}>نفذ</span>}
          {madeInMA&&<span style={{background:'rgba(34,197,94,0.2)',backdropFilter:'blur(8px)',color:'#22C55E',fontSize:10,fontWeight:700,padding:'4px 10px',borderRadius:DS.radiusFull,border:'1px solid rgba(34,197,94,0.3)'}}>🇲🇦 صنع في المغرب</span>}
        </div>
  ...
  return (
    <div onClick={()=>onView(p)} onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        ...
        <span style={{fontSize:11,fontWeight:600,background:`linear-gradient(135deg, ${DS.purple}, ${DS.purpleLight})`,color:'#fff',padding:'6px 16px',borderRadius:DS.radiusFull,boxShadow:`0 4px 12px ${DS.purpleGlow}`}}>احجز</span>
      </div>
    </div>
  );
}
...
function HeroSection({brand,onShop}:{brand:StoreInfo['brand'];onShop:()=>void}) {
  return (
    <div style={{position:'relative',zIndex:1,padding:'40px 20px 32px',textAlign:'center'}}>
      ...
      <h1 style={{fontSize:'clamp(24px,6vw,40px)',fontWeight:900,color:DS.textPrimary,margin:'0 0 8px',letterSpacing:'-0.03em',lineHeight:1.1,background:`linear-gradient(135deg, ${DS.textPrimary}, ${DS.purpleLight})`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text'}}>{brand.name||'المتجر'}</h1>
      ...
      <button onClick={onShop} style={{padding:'14px 36px',borderRadius:DS.radiusFull,background:`linear-gradient(135deg, ${DS.purple}, ${DS.purpleLight})`,border:'none',color:'#fff',fontSize:15,fontWeight:700,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:8,boxShadow:`0 8px 32px ${DS.purpleGlow}`,transition:DS.transitionFast,position:'relative'}} onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.transform='translateY(-2px)';(e.currentTarget as HTMLElement).style.boxShadow=`0 12px 40px ${DS.purpleGlow}`;}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.transform='';(e.currentTarget as HTMLElement).style.boxShadow=`0 8px 32px ${DS.purpleGlow}`;}}>استكشف المتجر <ArrowRight size={16}/></button>
    </div>
  );
}
    ...
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(8px)',zIndex:300,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:`linear-gradient(180deg, rgba(20,20,40,0.98), rgba(10,10,20,0.98))`,backdropFilter:DS.glassBlur,border:`1px solid ${DS.border}`,borderRadius:`${DS.radiusXl}px ${DS.radiusXl}px 0 0`,width:'100%',maxWidth:540,maxHeight:'90vh',overflowY:'auto',boxShadow:'0 -16px 64px rgba(0,0,0,0.5)',color:DS.textPrimary}}>
        ...
        <div style={{padding:'20px'}}>
          ...
          {(!p.type||p.type==='product')&&p.stock>0&&p.stock<=5&&<div style={{fontSize:12,color:'#EF4444',fontWeight:600,marginBottom:14,display:'flex',alignItems:'center',gap:6}}><Flame size={14}/> متبقي {p.stock} فقط في المخزون</div>}
          {p.description&&<p style={{fontSize:13,color:DS.textSecondary,lineHeight:1.7,marginBottom:20}}>{p.description}</p>}

          {p.colors?.length>0&&<div style={{marginBottom:18}}><div style={{fontSize:11,fontWeight:700,color:DS.textTertiary,marginBottom:10}}>اللون</div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}>{p.colors.map(clr=>{const CM:Record<string,string>={'أسود':'#1a1a1a','أبيض':'#f5f5f5','أحمر':'#ef4444','أزرق':'#3b82f6','أخضر':'#22c55e','رمادي':'#6b7280','بيج':'#d4b896','وردي':'#f472b6','بني':'#92400e','كحلي':'#1e3a5f','بنفسجي':'#a855f7','برتقالي':'#f97316'};return <button key={clr} onClick={()=>setColor(clr)} style={{width:36,height:36,borderRadius:'50%',background:CM[clr]||'#ccc',border:`3px solid ${color===clr?DS.purpleLight:'rgba(255,255,255,0.15)'}`,cursor:'pointer',transition:DS.transitionFast,boxShadow:color===clr?`0 0 16px ${DS.purpleGlow}`:'inset 0 0 0 1px rgba(0,0,0,0.1)'}} title={clr}/>;})}</div></div>}
          {p.sizes?.length>0&&<div style={{marginBottom:18}}><div style={{fontSize:11,fontWeight:700,color:DS.textTertiary,marginBottom:10}}>المقاس</div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}>{p.sizes.map(s=><button key={s} onClick={()=>setSize(s)} style={{padding:'10px 20px',borderRadius:DS.radiusFull,border:`1px solid ${size===s?DS.purpleLight:DS.border}`,background:size===s?DS.purpleSoft:DS.bgGlass,color:size===s?DS.purpleLight:DS.textSecondary,fontSize:13,fontWeight:600,cursor:'pointer',transition:DS.transitionFast,boxShadow:size===s?`0 0 12px ${DS.purpleGlow}`:'none'}}>{s}</button>)}</div></div>}

          <div style={{marginBottom:14}}>{variantRows.map((row,i)=><div key={i} style={{display:'flex',alignItems:'center',gap:10,marginBottom:8,padding:'12px 14px',background:DS.glassBg,borderRadius:DS.radiusMd,border:DS.glassBorder}}><span style={{fontSize:12,color:DS.textSecondary,minWidth:60}}>{row.color||'—'} / {row.size||'—'}</span><div style={{display:'flex',alignItems:'center',gap:6,marginRight:'auto'}}><button onClick={()=>setVariantRows(prev=>prev.map((r,j)=>j===i?{...r,qty:Math.max(1,r.qty-1)}:r))} style={{width:28,height:28,borderRadius:'50%',background:DS.bgGlass,border:DS.glassBorder,color:DS.textSecondary,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><Minus size={10}/></button><span style={{fontSize:14,fontWeight:700,color:DS.textPrimary,minWidth:20,textAlign:'center'}}>{row.qty}</span><button onClick={()=>setVariantRows(prev=>prev.map((r,j)=>j===i?{...r,qty:r.qty+1}:r))} style={{width:28,height:28,borderRadius:'50%',background:DS.bgGlass,border:DS.glassBorder,color:DS.textSecondary,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><Plus size={10}/></button></div><span style={{fontSize:14,fontWeight:700,color:DS.textPrimary,minWidth:50,textAlign:'right'}}>{(p.price*row.qty).toLocaleString()} {currency}</span>{variantRows.length>1&&<button onClick={()=>setVariantRows(prev=>prev.filter((_,j)=>j!==i))} style={{width:24,height:24,borderRadius:'50%',background:'rgba(239,68,68,0.15)',border:'none',color:'#EF4444',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><X size={10}/></button>}</div>)}</div>}
          {(p.sizes?.length>0||p.colors?.length>0)&&<button onClick={()=>setVariantRows(prev=>[...prev,{size:p.sizes?.[0]||'',color:p.colors?.[0]||'',qty:1}])} style={{padding:'10px 18px',borderRadius:DS.radiusFull,background:DS.bgGlass,border:`1px dashed ${DS.border}`,color:DS.textSecondary,fontSize:12,fontWeight:500,cursor:'pointer',display:'flex',alignItems:'center',gap:6,marginBottom:16,transition:DS.transitionFast}}><Plus size={12}/> أضف مقاس/لون آخر</button>}

          <div style={{marginBottom:16,padding:'14px',background:DS.glassBg,borderRadius:DS.radiusMd,border:DS.glassBorder}}><button onClick={()=>setGiftWrap(!giftWrap)} style={{display:'flex',alignItems:'center',gap:8,background:'none',border:'none',color:DS.textPrimary,fontSize:13,fontWeight:600,cursor:'pointer',padding:0}}><Gift size={16} color={giftWrap?'#10B981':DS.textTertiary}/>{giftWrap?'🎁 تغليف هدية (+15 درهم)':'هل هذه هدية؟ (+15 درهم)'}</button>{giftWrap&&<input placeholder="رسالة للهدية..." value={giftMsg} onChange={e=>setGiftMsgLocal(e.target.value)} style={{marginTop:10,width:'100%',padding:'10px 14px',borderRadius:DS.radiusSm,border:DS.glassBorder,background:DS.bgInput,color:DS.textPrimary,fontSize:12,outline:'none',boxSizing:'border-box',fontFamily:'Tajawal,sans-serif'}}/>}</div>

          {(()=>{const cfs=(p.customFields||[]).filter(f=>f.value&&!(f.type==='boolean'&&f.value!=='true'));if(!cfs.length)return null;const badges=cfs.filter(f=>f.type==='boolean');const longs=cfs.filter(f=>f.type==='textarea');const specs=cfs.filter(f=>f.type!=='boolean'&&f.type!=='textarea');return (<div style={{marginBottom:16,display:'flex',flexDirection:'column',gap:8}}>{badges.length>0&&<div style={{display:'flex',gap:6,flexWrap:'wrap'}}>{badges.map(f=><span key={f.id} style={{display:'flex',alignItems:'center',gap:5,fontSize:11,fontWeight:600,color:'#22C55E',background:'rgba(34,197,94,0.12)',border:'1px solid rgba(34,197,94,0.3)',padding:'5px 12px',borderRadius:DS.radiusFull}}><Check size={10}/>{f.label}</span>)}</div>}{specs.length>0&&<div style={{padding:'12px 14px',background:DS.glassBg,borderRadius:DS.radiusMd,border:DS.glassBorder}}><div style={{fontSize:10,color:DS.textTertiary,fontWeight:700,letterSpacing:'.06em',marginBottom:8}}>{p.type==='service'?'تفاصيل الخدمة':'تفاصيل المنتج'}</div><div style={{display:'flex',flexDirection:'column',gap:6}}>{specs.map(f=><div key={f.id} style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:10,fontSize:12}}><span style={{color:DS.textTertiary,flexShrink:0}}>{f.label}</span>{f.type==='multiselect'?<span style={{display:'flex',gap:4,flexWrap:'wrap',justifyContent:'flex-end'}}>{f.value.split('، ').filter(Boolean).map((v,i)=><span key={i} style={{fontSize:10,color:DS.textSecondary,background:DS.bgGlass,padding:'2px 9px',borderRadius:DS.radiusFull}}>{v}</span>)}</span>:<span style={{color:DS.textPrimary,fontWeight:600,textAlign:'left'}}>{f.value}</span>}</div>)}</div></div>}{longs.map(f=><div key={f.id} style={{padding:'12px 14px',background:DS.glassBg,borderRadius:DS.radiusMd,border:DS.glassBorder}}><div style={{fontSize:10,color:DS.textTertiary,fontWeight:700,letterSpacing:'.06em',marginBottom:6}}>{f.label}</div><p style={{fontSize:12,color:DS.textSecondary,lineHeight:1.7,margin:0,whiteSpace:'pre-wrap'}}>{f.value}</p></div>)}</div>);})()}
        ...
        <div style={{padding:'14px 20px 24px',position:'sticky',bottom:0,background:`linear-gradient(180deg, transparent, rgba(10,10,20,0.98) 20%)`,backdropFilter:DS.glassBlur,borderTop:`1px solid ${DS.border}`}}>
          <div style={{display:'flex',justifyContent:'space-between',marginBottom:10,fontSize:12,color:DS.textSecondary}}><span>{variantRows.reduce((s,r)=>s+r.qty,0)} قطعة</span><span>الإجمالي: {(p.price*variantRows.reduce((s,r)=>s+r.qty,0)+(giftWrap?15:0)).toLocaleString()} {currency}</span></div>
      ...
      <div onClick={e=>e.stopPropagation()} style={{width:'100%',maxWidth:360,background:`linear-gradient(180deg, rgba(25,25,50,0.99), rgba(12,12,25,0.99))`,border:`1px solid ${DS.borderPurple}`,borderRadius:DS.radiusXl,padding:'26px 22px',textAlign:'center',color:DS.textPrimary,boxShadow:`0 24px 80px rgba(0,0,0,0.6), 0 0 60px ${DS.purpleGlow}`}}>
        ...
        <div style={{position:'relative',width:260,height:260,margin:'0 auto 22px'}}>
          ...
          <div style={{width:'100%',height:'100%',borderRadius:'50%',position:'relative',overflow:'hidden',border:`5px solid rgba(255,255,255,0.12)`,boxShadow:`inset 0 0 30px rgba(0,0,0,0.4), 0 10px 40px rgba(0,0,0,0.5)`,background:`conic-gradient(${WHEEL_SEGMENTS.map((_,i)=>`${WHEEL_COLORS[i]} ${i*60}deg ${(i+1)*60}deg`).join(', ')})`,transform:`rotate(${rotation}deg)`,transition:spinning?'transform 4.2s cubic-bezier(0.15, 0.9, 0.18, 1)':'none'}}>
            {WHEEL_SEGMENTS.map((label,i)=><div key={i} style={{position:'absolute',top:'50%',left:'50%',transformOrigin:'0 0',transform:`rotate(${i*60+30}deg)`,width:0,height:0}}><span style={{position:'absolute',top:-105,left:0,transform:'translateX(-50%) rotate(0deg)',fontSize:10,fontWeight:800,color:'#fff',whiteSpace:'nowrap',textShadow:'0 1px 3px rgba(0,0,0,0.6)'}}>{label}</span></div>)}
        ...
        {result?(
          <div style={{marginBottom:6}}>
            ...
            {result.code&&<><div onClick={()=>{navigator.clipboard?.writeText(result.code!);}} style={{fontSize:18,fontWeight:900,letterSpacing:'.1em',color:DS.orangeLight,background:DS.glassBg,border:`1.5px dashed ${DS.borderOrange}`,borderRadius:DS.radiusSm,padding:'12px',marginBottom:8,cursor:'pointer',direction:'ltr'}}>{result.code}</div><div style={{fontSize:10,color:DS.textTertiary,marginBottom:12}}>صالح 48 ساعة · استخدام واحد{result.minOrder?` · لطلب فوق ${result.minOrder} درهم`:''} · أُضيف تلقائياً في السلة</div></>}
            <button onClick={onClose} style={{width:'100%',height:46,borderRadius:DS.radiusFull,background:`linear-gradient(135deg, ${DS.purple}, ${DS.purpleLight})`,border:'none',color:'#fff',fontSize:14,fontWeight:700,cursor:'pointer',boxShadow:`0 6px 24px ${DS.purpleGlow}`}}>{result.code?'تسوق واستعمل الكود 🛍️':'متابعة التسوق'}</button>
          </div>
        ):(
      ...
      <div style={{width:'min(420px,100vw)',background:`linear-gradient(180deg, rgba(20,20,40,0.98), rgba(10,10,20,0.98))`,backdropFilter:DS.glassBlur,display:'flex',flexDirection:'column',overflowY:'auto',boxShadow:'-16px 0 64px rgba(0,0,0,0.5)',borderLeft:`1px solid ${DS.border}`,color:DS.textPrimary}}>
        ...
        {step==='cart'&&(
          <div style={{flex:1,overflow:'auto',padding:18}}>
            {cart.items.length===0?(
              <div style={{textAlign:'center',padding:'80px 20px',color:DS.textTertiary}}><ShoppingBag size={48} style={{margin:'0 auto 16px',opacity:0.15}}/><div style={{fontSize:15,fontWeight:600,marginBottom:6}}>سلتك فارغة</div><button onClick={onClose} style={{marginTop:14,padding:'10px 28px',borderRadius:DS.radiusFull,background:`linear-gradient(135deg, ${DS.purple}, ${DS.purpleLight})`,border:'none',color:'#fff',fontWeight:600,fontSize:13,cursor:'pointer',boxShadow:`0 4px 20px ${DS.purpleGlow}`}}>تصفح المنتجات</button></div>
            ):(
              <>
                ...
                {(remainingForFreeShip>0||remainingForBundle>0||bestDiscount>0||freeShipping)&&<div style={{display:'flex',flexDirection:'column',gap:8,paddingTop:14}}>{freeShipping&&<div style={{fontSize:12,fontWeight:700,color:'#10B981',background:'rgba(16,185,129,0.08)',border:'1px solid rgba(16,185,129,0.25)',borderRadius:DS.radiusSm,padding:'10px 14px'}}>🚚 مبروك! توصيلك مجاني</div>}{!freeShipping&&remainingForFreeShip>0&&remainingForFreeShip<freeShipThreshold&&<div style={{background:DS.glassBg,border:DS.glassBorder,borderRadius:DS.radiusSm,padding:'10px 14px'}}><div style={{fontSize:11,fontWeight:600,color:DS.textSecondary,marginBottom:6}}>🚚 أضف <b style={{color:DS.orangeLight}}>{remainingForFreeShip.toLocaleString()} {cur}</b> ليصبح التوصيل مجانياً!</div><div style={{height:5,background:'rgba(255,255,255,0.08)',borderRadius:99,overflow:'hidden'}}><div style={{height:'100%',width:`${Math.min(100,Math.round(afterDiscount/freeShipThreshold*100))}%`,background:`linear-gradient(90deg, ${DS.purple}, ${DS.purpleLight})`,borderRadius:99,transition:'width .4s'}}/></div></div>}{bundleDiscount>0&&<div style={{fontSize:12,fontWeight:700,color:DS.purpleLight,background:DS.purpleSoft,border:`1px solid ${DS.borderPurple}`,borderRadius:DS.radiusSm,padding:'10px 14px'}}>🎁 خصم الباقة مفعّل: -{bundleDiscount.toLocaleString()} {cur} ({bundleCfg!.percent}%)</div>}{remainingForBundle>0&&bundleCfg&&<div style={{fontSize:11,fontWeight:600,color:DS.textSecondary,background:DS.glassBg,border:DS.glassBorder,borderRadius:DS.radiusSm,padding:'10px 14px'}}>🎁 أضف <b style={{color:DS.purpleLight}}>{remainingForBundle}</b> {remainingForBundle===1?'قطعة أخرى':'قطع أخرى'} واحصل على خصم <b style={{color:DS.purpleLight}}>{bundleCfg.percent}%</b> على كل السلة!</div>}</div>}
                <div style={{padding:'18px 0'}}><button onClick={()=>setStep('checkout')} style={{width:'100%',height:52,borderRadius:DS.radiusFull,background:`linear-gradient(135deg, ${DS.purple}, ${DS.purpleLight})`,border:'none',color:'#fff',fontSize:15,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:8,boxShadow:`0 8px 32px ${DS.purpleGlow}`}}>متابعة الطلب <ArrowRight size={16}/></button></div>
              </>
            )}
          </div>
        )}
        ...
        {step==='checkout'&&(
          <div style={{flex:1,overflow:'auto',padding:'18px 20px',display:'flex',flexDirection:'column',gap:12}}>
            ...
            <div><div style={{fontSize:12,fontWeight:700,color:DS.textTertiary,marginBottom:8}}>طريقة الدفع</div><div style={{display:'flex',gap:8}}>{[['cod','💵 عند الاستلام'],['virement','🏦 تحويل']].map(([v,l])=><button key={v} onClick={()=>setForm(f=>({...f,paymentMethod:v as any}))} style={{flex:1,padding:'12px',borderRadius:DS.radiusSm,border:`1px solid ${form.paymentMethod===v?DS.purpleLight:DS.border}`,background:form.paymentMethod===v?DS.purpleSoft:'transparent',color:form.paymentMethod===v?DS.purpleLight:DS.textSecondary,fontSize:12,fontWeight:600,cursor:'pointer',transition:DS.transitionFast}}>{l}</button>)}</div></div>
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(8px)',zIndex:350,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:`linear-gradient(180deg, rgba(20,20,40,0.98), rgba(10,10,20,0.98))`,backdropFilter:DS.glassBlur,border:`1px solid ${DS.border}`,borderRadius:DS.radiusXl,width:'100%',maxWidth:440,padding:28,boxShadow:DS.glassShadowHover,color:DS.textPrimary}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24}}><h2 style={{fontSize:18,fontWeight:900}}>📦 تتبع طلبك</h2><button onClick={onClose} style={{width:34,height:34,borderRadius:'50%',background:DS.glassBg,border:DS.glassBorder,color:DS.textSecondary,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><X size={14}/></button></div>
        <div style={{display:'flex',gap:8,marginBottom:14}}>{[['code','🔑 كود التتبع'],['phone','📱 رقم الهاتف']].map(([m,l])=><button key={m} onClick={()=>{setMode(m as any);setQuery('');setSearched(false);setSingleOrder(null);setOrders([]);}} style={{flex:1,padding:'10px',borderRadius:DS.radiusSm,border:`1px solid ${mode===m?DS.purpleLight:DS.border}`,background:mode===m?DS.purpleSoft:'transparent',color:mode===m?DS.purpleLight:DS.textSecondary,fontSize:12,fontWeight:600,cursor:'pointer',transition:DS.transitionFast}}>{l}</button>)}</div>
        <div style={{display:'flex',gap:8,marginBottom:18}}><input placeholder={mode==='code'?'أدخل كودك':'أدخل رقم هاتفك'} value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==='Enter'&&search()} dir="ltr" style={{flex:1,padding:'12px 15px',borderRadius:DS.radiusSm,border:DS.glassBorder,background:DS.bgInput,color:DS.textPrimary,fontSize:13,outline:'none',textTransform:mode==='code'?'uppercase':'none',fontFamily:'Tajawal,sans-serif'}}/><button onClick={search} disabled={loading} style={{padding:'8px 20px',borderRadius:DS.radiusSm,background:`linear-gradient(135deg, ${DS.purple}, ${DS.purpleLight})`,border:'none',color:'#fff',fontWeight:700,cursor:'pointer',fontSize:14}}>{loading?'⟳':'بحث'}</button></div>
...
export default function Storefront() {
  ...
  useEffect(()=>{const recentOrders=JSON.parse(localStorage.getItem('sahar_recent_orders')||'[]');if(recentOrders.length){const o=recentOrders[Math.floor(Math.random()*recentOrders.length)];const mins=Math.floor(Math.random()*12)+2;setLiveTicker([{name:o.name,city:o.city,product:o.product,time:`${mins} دقائق`}]);const tid=setTimeout(()=>setLiveTicker([]),5000);return()=>clearTimeout(tid);}},[cart.items.length]);
  ...
  if(!userId) return (<div dir="rtl" style={{minHeight:'100dvh',background:DS.bg,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:32,textAlign:'center',gap:16,fontFamily:'Tajawal,system-ui,sans-serif',color:DS.textPrimary}}><div style={{fontSize:48,opacity:0.3}}>🏪</div><div style={{fontSize:20,fontWeight:900}}>متجر SAHAR Shop</div><div style={{fontSize:13,color:DS.textTertiary,maxWidth:280,lineHeight:1.7}}>اطلب من التاجر مشاركة رابط متجره</div><a href="/" style={{padding:'10px 24px',borderRadius:DS.radiusFull,background:`linear-gradient(135deg, ${DS.purple}, ${DS.purpleLight})`,color:'#fff',fontWeight:700,fontSize:13,textDecoration:'none',marginTop:4,boxShadow:`0 4px 20px ${DS.purpleGlow}`}}>الصفحة الرئيسية</a></div>);

  if(error||(!loading&&!storeInfo)) return (<div dir="rtl" style={{minHeight:'100dvh',background:DS.bg,display:'flex',alignItems:'center',justifyContent:'center',color:DS.textPrimary,textAlign:'center',padding:32,fontFamily:'Tajawal,system-ui,sans-serif'}}><div><div style={{fontSize:36,marginBottom:12,opacity:0.3}}>🏪</div><div style={{fontSize:16,fontWeight:700,marginBottom:6}}>المتجر غير موجود</div><div style={{fontSize:13,color:DS.textTertiary}}>{error||'تحقق من الرابط'}</div></div></div>);

  const brand=storeInfo!.brand;
  const cur=brand.currency||'MAD';

  if(!loading&&!error&&storeInfo&&products.length===0) return (<div dir="rtl" style={{minHeight:'100dvh',background:DS.bg,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'32px 24px',textAlign:'center',fontFamily:'Tajawal,system-ui,sans-serif',color:DS.textPrimary}}><div style={{width:80,height:80,borderRadius:DS.radiusLg,background:DS.glassBg,backdropFilter:DS.glassBlur,border:DS.glassBorder,display:'flex',alignItems:'center',justifyContent:'center',marginBottom:20,boxShadow:DS.glassShadow}}>{brand.logo?<img src={brand.logo} alt="logo" style={{width:'100%',height:'100%',objectFit:'contain'}}/>:<span style={{fontSize:32}}>🏪</span>}</div><h1 style={{fontSize:24,fontWeight:900,marginBottom:8}}>{brand.name||'المتجر'}</h1><div style={{fontSize:60,margin:'20px 0 14px',opacity:0.1}}>📦</div><h2 style={{fontSize:18,fontWeight:600,marginBottom:8}}>المتجر قيد التجهيز</h2><p style={{fontSize:14,color:DS.textTertiary,maxWidth:300,lineHeight:1.8,marginBottom:28}}>ستضاف المنتجات قريباً — تابعونا!</p>{brand.phone&&<a href={`https://wa.me/${brand.phone.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" style={{padding:'12px 28px',borderRadius:DS.radiusFull,background:'#25D366',color:'#fff',fontSize:14,fontWeight:700,textDecoration:'none'}}>💬 تواصل معنا</a>}</div>);
